...
### 💰 LiteUI-Studio 是付费项目吗？
LiteUI-Studio 完全开源免费且没有付费内容，GitHub 发布地址为：[FJWRnoArina/LiteUI-Studio](https://github.com/FJWRnoArina/LiteUI-Studio/)。

### 🆚 与主流的 SD-webui 和 ComfyUI 有什么区别？
**SD-webui** 和 **ComfyUI** 对于入门用户都存在一定门槛：
*   **SD-webui**：需要用户手动设置底层模型、`CLIP`、`VAE`、负向提示词、引导强度、采样器、步数、`CFG Scale`、`Upscaler`、`Denoising Strength`、高清修复、`VAE Tile` 等繁杂参数，且显存管理不如 ComfyUI。
*   **ComfyUI**：入门门槛更高，需要熟知节点作用、模块化管理和复杂的节点连接逻辑。

**LiteUI-Studio** 以 ComfyUI API 为底层架构，继承了其优秀的显存管理，并通过 WebUI 的形式将复杂的文生图、图生图和视频生成工作流进行包装，极大简化了用户的使用体验。

### 🏗️ LiteUI-Studio 是基于 ComfyUI 架构吗？
**是的。** 位于 `/core/workflows` 目录中的文件都是可以直接在 ComfyUI 中打开的工作流。实际上，在 LiteUI-Studio 运行时，您可以直接在浏览器访问 http://127.0.0.1:8188/ 来打开底层的 ComfyUI。

### ⚙️ 为什么不自主设计模型加载架构？
这是 LiteUI-Studio 初始设计时的尝试。然而，整合各个开源项目最大的痛点是**兼容性问题**。
在实操代码时，我们发现 `transformers` 库、`xformer`、模型 pipeline lib、量化 lib 以及各种基础工具库（如 `torch`、`numpy` 甚至 Python 本身）之间的版本极难对齐，或者存在数层空映射，修复成本极高。ComfyUI 很好地解决了模型管线和量化之间的兼容性问题，这是我们选择它作为底层架构的重要原因。

### ⚖️ 支持生成任何内容吗？
生成的内容取决于用户选择的模型以及输入的提示词。请在**严格遵守本项目的免责声明与当地法律法规**的前提下使用 LiteUI-Studio。

### ❓ 遇到看不懂的选项、参数或报错怎么办？
*   **关于参数**：可以将想要查询的内容或截图发送至搜索引擎或 AI（如豆包、ChatGPT、Gemini 等）获取详细帮助。
*   **关于报错**：一些报错不会影响正常运行，可直接忽略；如果导致程序卡死或中断，请先尝试**重启 LiteUI-Studio**。若仍无法解决，可将报错截图发送给 AI 分析，或在 GitHub Issues 提交反馈。

### 📦 为什么无法加载我提供的模型或 LoRA？
*   **大模型限制**：为兼顾本地低显存运行与生成质量，LiteUI-Studio 目前（截至 2026 年 7 月）**仅支持**以下三个模型的 GGUF 量化版本（及其他微调模型）：
    *   `FLUX.2-klein-9B`
    *   `Wan2.2-I2V-A14B`
    *   `LTX-2.3`
    如需使用其他模型，请下载并使用原生的 SD-webui 或 ComfyUI。
*   **LoRA 限制**：请确保提供的 LoRA 必须适配上述三款模型之一。⚠️ **注意：** 生成图片的 LoRA 不能用于生成视频，反之亦然。

### 📝 我不知道怎么写提示词（Prompt）怎么办？
主流的 AI 助手（如豆包、ChatGPT、Gemini 等）都非常擅长代写 AI 绘画/视频提示词，直接向它们描述您的需求，然后复制生成的提示词使用即可。

### 🔍 如何追溯一张高质量图片/视频的生成参数？
将其上传至任何标准的 **Metadata 读取器** 即可查看生成时所用的参数信息。

### 📉 生成结果的质量不如预期怎么办？
为适配低显存设备，LiteUI-Studio 预装的模型为经过精简的**量化版本**，这在一定程度上会影响生成质量。您可以尝试：
1. 多次生成抽卡。
2. 优化/修改提示词。
3. 适当提高采样步数（Steps）。

### 🎬 同样是视频生成，Wan2.2 和 LTX2.3 有什么区别？
*   **Wan2.2**：本身不支持自带配音和对口型，但 LiteUI-Studio 为其额外装配了 `MMAudio` 配音模块（注：不支持人声台词）。
*   **LTX-2.3**：原生支持生成配音和台词，并能自动进行人物对口型。
---
