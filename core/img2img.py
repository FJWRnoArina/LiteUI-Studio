...
class FluxImg2ImgClient:
    ...
    def upload_image(self, image_path):
        ...
        if response.status_code == 200:
            ...
            print(f"[*] 图片上传成功，服务器命名为: {result['name']}")
            return result['name']
        else:
    ...
    def generate_img2img(self, template_path, input_image_path, lora_list=None, **kwargs):
        ...
        if "6" in workflow:
            ...
            raise Exception("模板中未找到 LoadImage 节点(6)")
        ...
        print(f"[*] 图生图任务已提交 (Prompt ID: {prompt_id})...")
                    ...
                    if isinstance(out, str):
                        ...
                        if message['type'] == 'executing':
                            ...
                            if 'out of memory' in error_msg.lower() or 'allocate' in error_msg.lower() or 'oom' in error_msg.lower():
                                raise Exception(f"💥 显存爆炸 (OOM)! 节点 [{node_id}] 显存不足。请尝试降低分辨率或减少 LoRA。")
                            elif "shapes cannot be multiplied" in error_msg.lower():
                                raise Exception(f"❌ 在节点 [{node_id}] 发生错误：模型加载失败！请确认按照提示加载了正确架构的模型。")
