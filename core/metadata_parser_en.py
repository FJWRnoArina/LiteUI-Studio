...
def extract_comfyui_metadata(file_path):
    ...
    # 2. 处理视频 (MP4, WEBM)
        ...
        try:
            ...
            return f"❌ Failed to extract metadata! Maybe ffprobe is missing or metadata is erased: {str(e)}"
...
def format_parsed_metadata(prompt):
    ...
    if wf_type == "txt2img":
        ...
        title_str = "🎞️🎵 Wan2.2 (Img2Vid + Audio)"
        ...
        data['high_base'] = get_val("257:258", "unet_name")
        data['low_base'] = get_val("257:259", "unet_name")
        title_str = "🎞️ Wan2.2 (Img2Vid No Audio)"
    ...
    for node_id, node in prompt.items():
        ...
        if class_type in ["LoraLoader", "LoraLoaderModelOnly"]:
            ...
            if lora_name: loras.append(f"{lora_name} (Weight: {inputs.get('strength_model', 1.0)})")
        elif class_type == "Power Lora Loader (rgthree)":
            for k, v in inputs.items():
                if k.startswith("lora_") and isinstance(v, dict) and v.get("on"):
                    loras.append(f"{v.get('lora')} (Weight: {v.get('strength')})")

    # 4. ================== 模块化 Markdown 排版 ==================
    md = f"### 📊 Generation Metadata\n"
    md += f"**Workflow:** `{title_str}`\n\n"

    # 渲染生图版参数
    if wf_type in ["txt2img", "img2img"]:
        md += f"- **📏 Resolution:** `{data.get('width')} x {data.get('height')}`\n"
        md += f"- **🎲 Seed:** `{data.get('seed')}`\n"
        md += f"- **👣 Steps:** `{data.get('steps')}`\n"
        md += f"- **🎛️ CFG/Guidance:** `{data.get('guidance')}`\n"

        if wf_type == "txt2img":
            md += f"- **🖼️ Batch Size:** `{data.get('batch_size')}`\n"
            md += f"- **🔍 Upscale:** `{data.get('upscale')}x`\n"

        md += f"\n#### 🧠 Model used\n"
        md += f"- **Based model:** `{data.get('base_model')}`\n"

    # 渲染 Wan2.2 视频参数
    elif wf_type.startswith("i2v"):
        md += f"- **📏 Resolution:** `{data.get('width')} x {data.get('height')}`\n"
        md += f"- **⏱️ Duration:** `{data.get('duration')}`\n"
        md += f"- **🎲 Seed:** `{data.get('seed')}`\n"

        if wf_type == "i2v_audio":
            md += f"- **🎵 Audio Seed:** `{data.get('audio_seed')}`\n"
            md += f"- **🔀 Separate Audio Prompt:** `{data.get('separate_audio')}`\n"

        md += f"\n#### 🧠 Model used\n"
        md += f"- **High Noise:** `{data.get('high_base')}`\n"
        md += f"- **Low Noise:** `{data.get('low_base')}`\n"
        if wf_type == "i2v_audio":
            md += f"- **Audio Model:** `{data.get('audio_model')}`\n"
    ...
    # 渲染 LTX-2.3 视频参数
        ...
        md += f"\n#### 🧠 Model used\n"
        md += f"- **Base model:** `{data.get('base_model')}`\n"
