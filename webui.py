...
import time
import requests
from core.txt2img import t2iClient
from core.image2vid import Wan2VideoPipelineClient
...
def open_output_folder(mode:str):
    ...
    try:
        ...
        raise gr.Error(f"打开文件夹失败: {str(e)}")

def interrupt_comfyui_task():
    """【全局急刹车】：向 ComfyUI 发送中断当前任务的 API 请求"""
    try:
        # ComfyUI 自带的硬中断接口
        requests.post(f"http://127.0.0.1:8188/interrupt", timeout=3)
        gr.Info("🛑 已向底层引擎发送紧急刹车指令！正在冲洗显存...")
    except Exception as e:
        gr.Warning("打断指令发送失败，请确保引擎在线。")
...
# ----- Txt2Img (Flux) 桥接函数 -----
def generate_t2i_wrapper(prompt, width, height, steps, guidance, upscale_factor, seed, randomize_seed, base_model, *lora_args):
    ...
    try:
        ...
        result_img = client.generate_txt2img(
            ...
            prompt=prompt, width=width, height=height, steps=steps, guidance=guidance, upscale_factor=upscale_factor, seed=seed,
            base_model=base_model
        )

        end_time = time.time()
        gr.Info(f"✅ 生成完毕！总用时{int(end_time - start_time)}秒")

        return result_img, seed

    except ConnectionRefusedError:
        raise gr.Error("❌ 引擎未响应：无法连接到 ComfyUI 后台！请确保后台服务已启动（默认端口 8188）。如果是刚打开整合包，请等待后台黑框加载完毕后再点击生成。")

    except FileNotFoundError:
        raise gr.Error("❌ 关键文件缺失！请检查文件完整性或重新安装。")

    except urllib.error.HTTPError:
        raise gr.Error("❌ 底层模型加载失败！请确认按照提示选择正确架构的模型。")
...
def add_lora_slot(count):
    ...
    if new_count == 5: gr.Warning("⚠️ 警告：加载 5 个以上的 LoRA 极易导致 8GB 显存 OOM！")
...
def generate_i2v_wrapper(image_path, prompt, duration, width, height, seed, randomize_seed,
    ...
    try:
        ...
        if image_path is None:
            raise gr.Error("❌ 必须上传一张初始图片才能生成视频！")
        ...
        gr.Info(f"🚀 已启动三阶段显存切片管线！\n当前设定: {duration}秒 (后台终端可查看阶段进度)")
...
def generate_ltx_wrapper(mode, image_path, prompt, width, height, duration, fps, seed, randomize_seed, base_model,
    ...
    try:
        start_time = time.time()
        # 判断模式
        is_t2v = (mode == "Text to Video (文生视频)")

        # 幽灵垫图机制：骗过 ComfyUI LoadImage 节点预检
        if image_path is None:
            if is_t2v:
                import PIL.Image
                ...
                raise gr.Error("❌ I2V 图生视频模式下，必须上传一张初始图片！")
        ...
        gr.Info(f"🎬 已启动 LTX-2.3 旗舰管线！\n耗时较长，请在终端关注 34B 参数极限解码进度...")
        ...
        raise gr.Error(
            "❌ 引擎未响应：无法连接到 ComfyUI 后台！请确保后台服务已启动（默认端口 8188）。如果是刚打开整合包，请等待后台黑框加载完毕后再点击生成。")
...
with (gr.Blocks(theme=theme, css=custom_css, title="LiteUI Studio") as demo):
    ...
    with gr.Tabs() as main_tabs:
        ...
        # TAB 1: 图像生成 (Flux)
        # ==========================================
        with gr.Tab("🖼️ 文生图 (Flux2)", id="tab_t2i"):
            with gr.Row():
                with gr.Column(scale=4):
                    t2i_prompt = gr.Textbox(label="Prompt (提示词)", lines=3, placeholder="支持使用自然语言提示词...")
                    with gr.Accordion("🛠️ 底层模型", open=False):
                        gr.HTML("""
                                <div style='background-color: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.3); padding: 8px; border-radius: 4px; margin-bottom: 8px;'>
                                    <span style='color: #ef4444; font-size: 0.9em; font-weight: bold;'>⚠️ 警告：请使用以Flux.2 Klein 9B为架构的GGUF模型，使用架构不匹配或后缀不是.gguf的模型可能导致生图管线崩溃。</span>
                                </div>
                                """)
                        t2i_base_model = gr.Dropdown(choices=unet_choices, value="flux-2-klein-9b-Q4_K_M.gguf",
                                                     label="Base Model (底模)", interactive=True)
                        t2i_model_open_folder_btn = gr.Button("📂 打开模型文件夹 (Open Folder)", size="lg")

                    with gr.Accordion("🛠️ 基础参数", open=True):
                        with gr.Row():
                            t2i_width = gr.Slider(label="Width (宽)", minimum=512, maximum=2048, step=16, value=1024, scale=10)
                            t2i_swap_btn = gr.Button("🔄", elem_classes="swap-btn", scale=1)
                            t2i_height = gr.Slider(label="Height (高)", minimum=512, maximum=2048, step=16, value=1024,
                                                   scale=10)
                        with gr.Row():
                            t2i_steps = gr.Slider(label="采样步数", minimum=1, maximum=20, step=1, value=4)
                            t2i_guidance = gr.Slider(label="提示词引导强度", minimum=1.0, maximum=10.0, step=0.1, value=4.0)
                        with gr.Row():
                            t2i_upscale_factor = gr.Slider(label="分辨率放大比例", minimum=1, maximum=10, step=0.05, value=1, info="建议不超过3.00。")
                        with gr.Row():
                            t2i_seed = gr.Number(label="Seed", value=88888888, precision=0)
                            t2i_rand_seed = gr.Checkbox(label="🎲 随机", value=True)

                    with gr.Accordion("🪄 LoRA 模型管理", open=False):
                        ...
                        for i in range(MAX_LORAS):
                            with gr.Row(visible=(i == 0)) as row:
                                ...
                                weight = gr.Slider(minimum=-2.0, maximum=2.0, step=0.05, value=1.0, label="权重",
                                                   scale=3)
                        ...
                        add_lora_btn = gr.Button("➕ 添加 LoRA", size="sm")
                ...
                with gr.Column(scale=5):
                    t2i_output_img = gr.Image(label="生成的图像 (Result)", format="png", width=800,height=600, interactive=False)
                    t2i_output_seed = gr.Number(label="Seed", interactive=False)
                    with gr.Row():
                        send_to_i2i_btn = gr.Button("📤 一键发送至图生图 (Send to Img2Img)", size="lg")
                        send_to_i2v_btn = gr.Button("📤 一键发送至视频生成 (Send to Img2Vid)", size="lg")
                        t2i_open_folder_btn = gr.Button("📂 打开输出文件夹", size="lg", scale=1)

        # ==========================================
        # TAB 2: I2I (Flux)
        # ==========================================
        with gr.Tab("🎨 图生图 (Flux2)", id="tab_i2i"):
            with gr.Row():
                with gr.Column(scale=4):
                    # type="filepath" 会自动保存上传的图片供后端读取
                    i2i_image = gr.Image(label="Input Image (参考图)", type="filepath", height=256)
                    i2i_prompt = gr.Textbox(label="Prompt (提示词)", lines=3, placeholder="支持使用自然语言提示词...")
                    ...
                    with gr.Accordion("🛠️ 底层模型", open=False):
                        ...
                        i2i_base_model = gr.Dropdown(choices=unet_choices, value="flux-2-klein-9b-Q4_K_M.gguf",
                                                     label="Base Model (底模)", interactive=True)
                        i2i_model_open_folder_btn = gr.Button("📂 打开模型文件夹 (Open Folder)", size="lg")
                    ...
                    with gr.Accordion("🛠️ 基础参数", open=True):
                        ...
                        with gr.Row():
                            ...
                            i2i_guidance = gr.Slider(label="提示词引导强度", minimum=1.0, maximum=10.0, step=0.1, value=4.0)
                        with gr.Row():
                            i2i_seed = gr.Number(label="Seed", value=88888888, precision=0)
                            i2i_rand_seed = gr.Checkbox(label="🎲 随机", value=True)
                    ...
                    with gr.Accordion("🪄 LoRA 模型管理", open=False):
                        ...
                        i2i_add_lora_btn = gr.Button("➕ 添加 LoRA", size="sm")
                ...
                with gr.Column(scale=5):
                    i2i_output_img = gr.Image(label="生成的图像 (Result)", format="png", width=800,height=600,interactive=False)
                    i2i_output_seed = gr.Number(label="Seed", interactive=False)
                    with gr.Row():
                        send_i2i_to_i2v_btn = gr.Button("📤 一键发送至视频生成 (Send to Img2Vid)", size="lg")
                        i2i_open_folder_btn = gr.Button("📂 打开输出文件夹", size="lg", scale=1)

        # ==========================================
        # TAB 3: 视频生成 (Wan2.2)
        # ==========================================
        with gr.Tab("🎞️ 图生视频 (Wan2.2)", id="tab_i2v"):
            with gr.Row():
                with gr.Column(scale=4):
                    i2v_image = gr.Image(label="Input Image (初始帧图像)", type="filepath", height=256)
                    i2v_prompt = gr.Textbox(label="Video Prompt (画面提示词)", lines=2,
                                            placeholder="支持使用自然语言提示词...")

                    with gr.Accordion("🛠️ 底层模型高级设置 (Advanced Models)", open=False):
                        gr.HTML("""
                        <div style='background-color: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.3); padding: 8px; border-radius: 4px; margin-bottom: 8px;'>
                            <span style='color: #ef4444; font-size: 0.9em; font-weight: bold;'>⚠️ 警告：如果不熟悉工作流原理，请务必使用默认选项！随意切换模型可能导致管线崩溃。</span>
                        </div>
                        """)
                        i2v_high_base = gr.Dropdown(choices=unet_choices, value="Wan2.2-I2V-A14B-HighNoise-Q4_K_M.gguf",
                                                    label="High Noise Base Model (高噪底模)", interactive=True)

                        i2v_low_base = gr.Dropdown(choices=unet_choices, value="Wan2.2-I2V-A14B-LowNoise-Q4_K_M.gguf",
                                                   label="Low Noise Base Model (低噪底模)", interactive=True)
                        # 这个音频底模也受总开关控制，默认情况被锁死
                        i2v_audio_model = gr.Dropdown(choices=mmaudio_choices,
                                                      value="mmaudio_large_44k_v2_fp16.safetensors",
                                                      label="MMAudio Model (音频模型)", interactive=True)

                        i2v_model_open_folder_btn = gr.Button("📂 打开模型文件夹 (Open Folder)", size="lg")
                        i2v_mmaudio_open_folder_btn = gr.Button("📂 打开MMAudio文件夹 (Open Folder)", size="lg")

                    with gr.Accordion("⚙️ 视频规格设置", open=True):
                        with gr.Row():
                            i2v_width = gr.Slider(label="Width (宽)", minimum=256, maximum=1024, step=16, value=512,
                                                  scale=10)
                            i2v_swap_btn = gr.Button("🔄", elem_classes="swap-btn", scale=1)
                            i2v_height = gr.Slider(label="Height (高)", minimum=256, maximum=1024, step=16, value=768,
                                                   scale=10)
                        with gr.Row():
                            i2v_duration = gr.Slider(label="Duration (时长)", minimum=1, maximum=30, step=1, value=5,
                                                     info="💡 5秒约 5-6 分钟，10秒约 14-15 分钟")
                            # fps 选项已被架构师安全抹除 🐇
                        with gr.Row():
                            i2v_seed = gr.Number(label="High Noise Seed", value=1234567, precision=0)
                            i2v_rand_seed = gr.Checkbox(label="🎲 随机", value=True)

                    with gr.Accordion("🪄 Wan2.2 LoRA (High/Low)", open=False):
                        gr.Markdown("Wan2.2 是双轨架构，需分别配置高噪/低噪 LoRA。")
                        with gr.Row():
                            i2v_high_lora = gr.Dropdown(choices=lora_choices, value="None", label="High Noise LoRA",
                                                        scale=3)
                            i2v_high_str = gr.Slider(minimum=0.0, maximum=2.0, step=0.05, value=1.0, label="权重",
                                                     scale=1)
                        with gr.Row():
                            i2v_low_lora = gr.Dropdown(choices=lora_choices, value="None", label="Low Noise LoRA",
                                                       scale=3)
                            i2v_low_str = gr.Slider(minimum=0.0, maximum=2.0, step=0.05, value=1.0, label="权重",
                                                    scale=1)

                        i2v_lora_open_folder_btn = gr.Button("📂 打开LoRA文件夹 (Open Folder)", size="lg")

                    with gr.Accordion("🎵 视频配音 (Audio Generation)", open=True):
                        i2v_enable_audio = gr.Checkbox(label="🔊 启用视频配音 (Enable Audio Engine)", value=False)

                        # 【修改 1】：在 label 里混入 HTML 的 span 标签，利用 title 属性实现鼠标悬浮提示
                        i2v_separate_audio = gr.Checkbox(
                            label="🔀 分离配音提示词 (Separate Audio Prompt)",
                            info="禁用时，系统将默认使用画面提示词作为配音提示词",
                            value=False,
                            interactive=False
                        )
                        ...
                        i2v_audio_prompt = gr.Textbox(
                            label="Audio Prompt (音效提示词)",
                            lines=2,
                            placeholder="请输入以逗号分隔的英文提示词（可以让AI代写）...",
                            interactive=False
                        )
                    ...
                    <div style='background-color: rgba(245, 158, 11, 0.15); border: 1px solid rgba(245, 158, 11, 0.3); padding: 12px; border-radius: 6px; margin-bottom: 10px; line-height: 1.6;'>
                        <span style='color: #f59e0b; font-weight: bold; font-size: 1em;'>⏳ 硬件极限警告：</span>
                        <span style='color: var(--body-text-color); font-size: 0.9em;'>视频生成极其消耗算力。</span><br>
                        <span style='color: var(--body-text-color); font-size: 0.9em;'>点击生成后请勿刷新页面。如开启音频生成，耗时将增加 20%~30%。请通过控制台观察底层计算进度。</span>
                    </div>
                    """)
                    i2v_btn = gr.Button("🎬 生成视频 (Generate Video)", variant="primary", size="lg")
                    i2v_stop_btn = gr.Button("🛑 打断", variant="stop", size="lg", scale=1)

                with gr.Column(scale=5):
                    i2v_output_vid = gr.Video(label="生成的最终视频 (Result)", width=800,height=600,interactive=False)
                    i2v_output_seed = gr.Number(label="Seed", interactive=False)

                    i2v_open_folder_btn = gr.Button("📂 打开本地输出文件夹 (Open Folder)", size="lg")

        # ==========================================
        # TAB 5: LTX-2.3 (Native Audio-Video)
        # ==========================================
        with gr.Tab("🌟 LTX-2.3 (原生多模态视频)", id="tab_ltx"):
            with gr.Row():
                with gr.Column(scale=4):
                    # 模式选择器
                    ltx_mode = gr.Radio(
                        choices=["Text to Video (文生视频)", "Image to Video (图生视频)"],
                        value="Text to Video (文生视频)",
                        label="🎥 生成模式 (Generation Mode)"
                    )

                    # 图片上传框 (默认隐藏，只有选了 I2V 才弹出来)
                    ltx_image = gr.Image(label="Input Image (初始帧图像)", type="filepath", height=256,
                                         visible=False)

                    ltx_prompt = gr.Textbox(
                        label="🎬 导演时间轴提示词 (Temporal Prompt)",
                        lines=7,
                        ...
                        info="支持以秒为单位指定动作与原声台词！"
                    )

                    with gr.Accordion("⚙️ 视频规格设置", open=True):
                        with gr.Row():
                            ltx_width = gr.Slider(label="Width (宽)", minimum=256, maximum=1024, step=16,
                                                  value=512,
                                                  scale=10)
                            ltx_swap_btn = gr.Button("🔄", elem_classes="swap-btn", scale=1)
                            ltx_height = gr.Slider(label="Height (高)", minimum=256, maximum=1024, step=16,
                                                   value=768, scale=10)
                        with gr.Row():
                            ltx_duration = gr.Slider(label="Duration (时长/秒)", minimum=1, maximum=20, step=1,
                                                     value=10)
                            ltx_fps = gr.Slider(label="FPS (帧率)", minimum=8, maximum=60, step=1, value=24)
                        with gr.Row():
                            ltx_seed = gr.Number(label="Base Seed (基础种子)", value=88888888, precision=0)
                            ltx_rand_seed = gr.Checkbox(label="🎲 随机", value=True)

                    with gr.Accordion("🛠️ 底层模型与 LoRA", open=False):
                        gr.HTML("""
                                <div style='background-color: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.3); padding: 8px; border-radius: 4px; margin-bottom: 8px;'>
                                    <span style='color: #ef4444; font-size: 0.9em; font-weight: bold;'>⚠️ 警告：如果不熟悉工作流原理，请务必使用默认选项！随意切换模型可能导致管线崩溃。</span>
                                </div>
                        ...
                        ltx_base_model = gr.Dropdown(choices=unet_choices,
                                                     ...
                                                     label="LTX Base Model (底模)", interactive=True)
                    ...
                    gr.HTML("""
                        <div style='background-color: rgba(245, 158, 11, 0.15); border: 1px solid rgba(245, 158, 11, 0.3); padding: 12px; border-radius: 6px; margin-bottom: 10px; line-height: 1.6;'>
                            <span style='color: #f59e0b; font-weight: bold; font-size: 1em;'>⏳ 旗舰管线警告：</span>
                            <span style='color: var(--body-text-color); font-size: 0.9em;'>此模型同时渲染高分辨率画面与原声配音。</span><br>
                            <span style='color: var(--body-text-color); font-size: 0.9em;'>点击生成后请勿刷新页面，终端监控会有明显发热，请耐心等待！</span>
                        </div>
                        """)
                    ltx_btn = gr.Button("🎬 启动 LTX 导演引擎 (Generate)", variant="primary", size="lg")
                    ltx_stop_btn = gr.Button("🛑 打断", variant="stop", size="lg", scale=1)
        ...
        with gr.Tab("🔍 Metadata读取器 (Metadata Inspector)", id="tab_inspector"):
            ...
            with gr.Row():
                with gr.Column(scale=4):
                    ...
                    inspector_input = gr.File(label="📁 上传图像或视频文件", file_types=["image", "video"],
                                              type="filepath")
                    mi_open_folder_btn = gr.Button("📂 打开本地输出文件夹 (Open Folder)", size="lg")

                with gr.Column(scale=5):
                    # 用 Markdown 组件来渲染我们排版好的文字结果
                    inspector_output = gr.Markdown(value="*参数将在这里显示...*", elem_classes="markdown-output")
    ...
    def toggle_ltx_mode(mode_val):
        ...
        return gr.update(visible=(mode_val == "Image to Video (图生视频)"))
